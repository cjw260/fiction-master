...
flowchart LR
    ...
    H --> K["Qwen Rerank"]
    K --> G["证据充分性检查"]
    G -->|"不足，最多一次"| M["生成 2-3 条互补查询"]
    M --> B["批量 Embedding + 分路混合召回"]
    B --> J["RRF 合并去重"]
    J --> K
    G --> A["基于原文生成带编号回答"]
...
关系型或全局问题在首轮检索时还会启动可选的 LightRAG 旁路：LightRAG 返回实体、
关系和关联章节线索，系统将这些线索转换成补充查询后重新搜索当前 Qdrant active
version，再以较低权重参与 RRF 和统一重排。LightRAG 的模型摘要不会直接进入最终
证据，因而现有原文引用约束保持不变。
...
- LightRAG 不可用或图谱未就绪时降级到原有检索；主索引和书籍 ready 状态不受影响。
- 图谱章节来源携带 `book_id/index_version/chapter_ordinal`，查询只接受当前活动版本。
...
- 混合检索、书目路由、证据不足时的一次多查询重检、流式回答和原文引用。
...
- `.env` 密钥不进入 Git。小说原文、SQLite、Qdrant 与 LightRAG 的完整现有数据快照
  保存在 `backups/dataset/` 并随仓库提交；运行目录本身仍被忽略。
